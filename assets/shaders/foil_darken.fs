...
extern MY_HIGHP_OR_MEDIUMP vec2 foil_darken;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec2 rotater = vec2(cos(foil_darken.r*0.1221), sin(foil_darken.r*0.3512));
    number angle = dot(rotater, adjusted_uv)/(length(rotater)*length(adjusted_uv));
    number fac2 = max(min(5.*cos(foil_darken.g*0.3 + angle*3.14*(2.2+0.9*sin(foil_darken.r*1.65 + 0.2*foil_darken.g))) - 4. - max(2.-length(20.*adjusted_uv), 0.), 1.), 0.);
    number fac3 = 0.3*max(min(2.*sin(foil_darken.r*5. + uv.x*3. + 3.*(1.+0.5*cos(foil_darken.r*7.))) - 1., 1.), -1.);
    number fac4 = 0.3*max(min(2.*sin(foil_darken.r*6.66 + uv.y*3.8 + 3.*(1.+0.5*cos(foil_darken.r*3.414))) - 1., 1.), -1.);
